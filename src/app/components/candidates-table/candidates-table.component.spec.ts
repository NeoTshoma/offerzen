...
describe('CandidatesTableComponent', () => {
  ...
  it('should sort the list ascendingly by date', () => {
    component.sort('ASC');

    expect(component.candidatesList[0].candidate).toEqual('Nkosana Ndebele');
  });

  it('should sort the list descendingly by date', () => {
    component.sort('DESC');

    expect(component.candidatesList[0].candidate).toEqual('Natasha Peters');
  })
});
